...
from models.tit.tcn import TCN
from models.tit.MLP import MLP_ChannelTokens, MLP_TimeTokens, MLPMixer, Linear
from models.tit.rnn import RNNet
from models.tit.lstm import LSTMnet
# from models.tit.TiT import TiT
# from models.tit.nhits import NHits
from models.tit.DLinear import Model as DLinear
from models.evolution.TransformerEvolution import Transformer_TimeTokens_noSeg, Transformer_ChannelTokens_noSeg
...
parser.add_argument('--pre_embed', type=str, default='none', help='pre-embedding method')
...
# decoder ablation
parser.add_argument('--dec_name', type=str, default='linear')
parser.add_argument('--dec_cross', type=str, default='att')
parser.add_argument('--dec_self', type=str, default='att')
parser.add_argument('--embedding', type=str, default='none', help='embeddings for transformer')
...
if args.model_type == 'TCN':
    ...
    expname = '{}/{}/{}_{}_in{}out{}_lr{}_bs{}_blocks{}_d{}_dropout{}_epoch{}' \
        .format(args.data_path[:-4], args.model_type, \
            args.exp_name, args.features, args.seq_len, args.pred_len, \
                args.learning_rate, args.batch_size, args.layer,args.d_model, args.dropout, args.train_epochs)
...
#     model = TiT(args)
#     expname = '{}/{}/{}/{}_{}_{}_{}_{}_in{}d{}out{}_lr{}_layers{}_numHead{}_dropout{}_epoch{}_seg{}_lradj{}_opt{}_act{}_initg{}_peg{}' \
...
#                 args.learning_rate, args.layer, args.n_heads, args.dropout, args.train_epochs, args.segment, args.lradj, args.optimizer, \
#                     args.activation, args.init_gain, args.pe_gain)


# elif args.model_type.lower() == 'nhits':
#     model = NHits(args)
#     expname = '{}/{}/{}/{}_{}_{}_{}_in{}out{}_lr{}_bs{}_layers{}_numHead{}_dropout{}_epoch{}' \
#         .format(args.data_path[:-4], args.model_type, args.exp_name[:4], \
#             args.exp_name[5:], args.attentionlayer, args.timelayer, args.features, args.seq_len, args.pred_len, \
#                 args.learning_rate, args.batch_size, args.layer, args.n_heads, args.dropout, args.train_epochs)


elif args.model_type == 'rnn':
    model=RNNet(num_layers=args.rnn_layers,seq_len=args.seq_len,pred_len=args.pred_len, num_channels=args.enc_in)
    expname = '{}/{}/{}_{}_in{}out{}_layer{}_lr{}_bs{}_epoch{}' \
        .format(args.data_path[:-4], args.model_type, \
            args.exp_name, args.features, args.seq_len, args.pred_len, args.rnn_layers, \
                args.learning_rate, args.batch_size, args.train_epochs)

elif args.model_type == 'lstm':
    model=LSTMnet(num_layers=args.rnn_layers,seq_len=args.seq_len,pred_len=args.pred_len, num_channels=args.enc_in)
    expname = '{}/{}/{}_{}_in{}out{}_layer{}_lr{}_bs{}_epoch{}' \
        .format(args.data_path[:-4], args.model_type, \
            args.exp_name, args.features, args.seq_len, args.pred_len, args.rnn_layers, \
                args.learning_rate, args.batch_size, args.train_epochs)
    ...
    model=Linear(args.seq_len, args.pred_len, args.layer)
    expname = '{}/{}/{}_{}_in{}out{}_layer{}_lr{}_bs{}_epoch{}' \
        .format(args.data_path[:-4], args.model_type, \
            args.exp_name, args.features, args.seq_len, args.pred_len, args.layer, \
                args.learning_rate, args.batch_size, args.train_epochs)

                
elif args.model_type == 'DLinear':
    model = DLinear(args)
    expname = '{}/{}/{}_{}_in{}out{}_lr{}_bs{}_epoch{}' \
        .format(args.data_path[:-4], args.model_type, \
            args.exp_name, args.features, args.seq_len, args.pred_len, \
                args.learning_rate, args.batch_size, args.train_epochs) 
...
def vali(vali_data, vali_loader, criterion, epoch, writer, flag='vali'):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            ...
            if beyond_len > 0:
                outputs = outputs[:, :-beyond_len, :]
                batch_y = batch_y[:, :-beyond_len, :]
...
def train():
    ...
    best_test_loss = 114514.0
    best_train_loss = 114514.0
    best_epoch = -1
    ...
    for epoch in range(args.train_epochs):
        ...
        vali_loss = vali(vali_data, vali_loader, criterion, epoch, writer, 'vali')
        test_loss = vali(test_data, test_loader, criterion, epoch, writer, 'test')
        ...
        print('Best Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} Test Loss: {3:.7f}'.format(
    ...
    f.write('\n{} \n'.format(expname))
    f.write('Best Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} Test Loss: {3:.7f} \n'.format(
        best_epoch, best_train_loss, best_vali_loss, best_test_loss
    ))
...
def test(setting='setting',test=1):
    ...
    origins = origins.reshape(-1, origins.shape[-2], origins.shape[-1])
    print('test shape:', preds.shape, trues.shape, origins.shape)

    # result save
    folder_path = os.path.join('./results/test', expname, 'metrics/')
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    ...
    np.save(folder_path + 'origin.npy', origins)

...
parser.add_argument('--layer',type=int,default=2,help='layer of block')
# MLP hidden
...
def vali(vali_data, vali_loader, criterion, metric, epoch, writer, flag='vali'):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            ...
            if args.output_attention and flag == 'test' and i == 0:
                torch.save([pred, true], os.path.join(save_path, 'TESTtimeseries.pt'))
...
def train():
    ...
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    f = open(folder_path + '/{}_{}_{}_exps.txt'.format(args.exp_name, args.model_type, args.dec_name), 'a')
    f.write('\n{} \n'.format(expname))

...
class Transformer_TimeTokens(nn.Module):
    def __init__(self, args):
        super(Transformer_TimeTokens, self).__init__()
        ...
        d_model = args.enc_in * args.segment_len
        # structure, encoder
        ...
        if args.dec_name == 'transformer':
            self.decoder = TransformerDecoder(dim=d_model, depth=args.d_layers, heads=args.n_heads, dim_head=d_model, \
                mlp_dim=d_model, dropout=args.dropout, dec_self=args.dec_self, dec_cross=args.dec_cross, output_attention=self.output_attention)
        elif args.dec_name == 'linear':
    ...
    def forward(self, x, x_mark, dec_inp, y_mark, ratio=False): #[B, L, C]
        ...
        enc_in = rearrange(enc_in, 'b (n s) c -> b n (s c)', s=self.segment_len)
        ...
        if self.dec_name == 'transformer':
            ...
            dec_in = rearrange(dec_in, 'b (n s) c -> b n (s c)', s=self.segment_len)
            if not self.output_attention:
                dec_out = self.decoder(dec_in, enc_out, enc_out)
            else:
                dec_out, dec_self_atts, dec_cross_atts = self.decoder(dec_in, enc_out, enc_out)
            dec_out = rearrange(dec_out, 'b n (s c) -> b (n s) c', s=self.segment_len)
        elif self.dec_name == 'linear':
            enc_out = rearrange(enc_out, 'b n (s c) -> b c (n s)', s=self.segment_len)
            dec_in = enc_out
...
class Transformer_ChannelTokens(nn.Module):
    def __init__(self, args):
        super(Transformer_ChannelTokens, self).__init__()
    ...
    def forward(self, x, x_mark, dec_inp, y_mark): #[B, L, C]
        ...
        enc_out = rearrange(enc_out, 'b (n s) c -> b (n c) s', s=self.segment_len)
        if not self.output_attention:
            enc_out = self.encoder(enc_out)
        else:
            enc_out, enc_atts = self.encoder(enc_out)

        if self.dec_name == 'transformer':
            # enc_out = rearrange(enc_out, '(b n) c s -> b c (n s)', b=self.batch_size)
            # enc_out = self.enc_projection(enc_out)
            # enc_out = rearrange(enc_out, 'b c (n s) -> (b n) c s', s=self.segment_len)

            dec_out = self.dec_embedding(dec_inp, y_mark)
            if self.learnable_query:
                dec_out = dec_out + self.queries
            dec_out = rearrange(dec_out, 'b (n s) c -> b (n c) s', s=self.segment_len)
            if not self.output_attention:
                dec_out = self.decoder(dec_out, enc_out, enc_out)
            else:
                dec_out, dec_self_atts, dec_cross_atts = self.decoder(dec_out, enc_out, enc_out)
            dec_out = rearrange(dec_out, 'b (n c) s -> b (n s) c', c=self.channels)
        elif self.dec_name == 'linear':
            enc_out = rearrange(enc_out, 'b (n c) s -> b c (n s)', c=self.channels)
